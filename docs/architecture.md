...
| 20 | Scope is two bars, seniority and domain, and a refusal records which one failed | Accepted | The segments are seniority definitions; the channel matrix behind them was researched for senior technology talent. A Director of Corporate Strategy cleared triage and was pointed at the CIO Association of Canada. Recording the axis matters because the practice intends to open to other functions, and the volume turned away on domain grounds is the signal for when | Revisited when the practice opens to other functions: the domain bar moves, the two-bar structure does not |
| 21 | Search strings are generated against the channel plan, not independently of it | Accepted | A live run produced a code host X-ray for a corporate strategy executive while the same output listed the code host as skipped. A search string for a channel the plan tells you to skip is a contradiction inside one deliverable | Foundational |
| 18 | A constraint declares whether it was stated or inferred, and an inference never narrows the market | Accepted | A live run marked "NERC CIP work typically carries personnel risk assessment obligations" as disqualifying, against a quote that is real but does not establish it. The insight is useful and the specification says no such thing; unflagged it shrank the pipeline arithmetic on the model's background knowledge. Inferences are surfaced for confirmation instead | Foundational |
| 19 | Every source quote is verified verbatim against the document it claims to come from | Accepted | The value of a quote is that a recruiter can check a constraint against the source rather than trust a paraphrase, and a quote that is not in the document defeats that while looking more trustworthy than no quote at all. Catches fabrication, which the inferred flag cannot; the inferred flag catches non-probative quotes, which verification cannot | Foundational |
